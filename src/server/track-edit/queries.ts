...
export const getTrackEditCore = cache(
  async (id: string): Promise<TrackEditCoreDTO | null> => {
...
export const getTrackAudioModule = cache(
  async (id: string): Promise<TrackAudioModuleDTO | null> => {
    return prisma.track.findUnique({
      where: { id },
      select: {
        id: true,
...
export const getTrackAudioHeaderModule = cache(
  async (id: string): Promise<TrackAudioHeaderDTO | null> => {
    return prisma.track.findUnique({
      where: { id },
      select: {
        id: true,
...
export const getTrackRightsModule = cache(
  async (id: string): Promise<TrackRightsModuleDTO | null> => {
    return prisma.track.findUnique({
      where: { id },
      select: {
        id: true,
...
export const getTrackDeliverablesModule = cache(
  async (id: string): Promise<TrackDeliverablesModuleDTO | null> => {
    return prisma.track.findUnique({
      where: { id },
      select: {
        id: true,
...
export const getCatalogTagOptions = cache(
  async (): Promise<CatalogTagOptionDTO[]> => {
...
export const getMoodTagOptions = cache(
  async (): Promise<MoodTagOptionDTO[]> => {
    return prisma.tag.findMany({
      where: { type: "MOOD" },
      select: { id: true, slug: true, name: true },
      orderBy: { name: "asc" },
...
export const getTrackAssetsPageData = cache(async (id: string) =>
  prisma.track.findUnique({
    ...
    select: {
      ...
      stems: {
        select: { name: true, group: true, durationSec: true },
        orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
      },
      versions: {
        select: { label: true, kind: true, durationSec: true },
