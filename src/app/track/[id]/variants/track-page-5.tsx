...
export default async function TrackPublicPage({ params }: PageProps) {
  ...
  const guidePrice = licenseOptions[0]?.price ?? "A cotizar";
  const licenseLabel = formatLicenseType(track.licenseType);
  ...
  return (
    <div className="bg-background text-foreground">
      <div className="mx-auto w-[90vw] max-w-[1700px] pb-20 pt-3 sm:pt-4">
        ...
        <section className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_330px] xl:items-start">
          <div className="space-y-3">
            <article className="overflow-hidden rounded-md border border-border bg-card/30">
              <div className="grid min-w-0 sm:grid-cols-[188px_minmax(0,1fr)]">
                <div className="h-full border-b border-border sm:border-b-0 sm:border-r">
                  <img
                    src={coverUrl}
                ...
                <div className="p-3 sm:p-4">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                    Track page 5 · premium minimal
                  </p>
                  <h1 className="mt-1 text-2xl font-semibold leading-tight sm:text-3xl">{track.title}</h1>
                  <p className="mt-0.5 text-sm text-muted-foreground sm:text-base">{track.artist || "Artista"}</p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    <span title="Beats por minuto del track" className="rounded border border-border bg-background/80 px-2 py-0.5 text-xs font-semibold text-foreground">
                      BPM {track.bpm ? Math.round(track.bpm) : "—"}
                    </span>
                    <span title="Tonalidad principal" className="rounded border border-border bg-background/80 px-2 py-0.5 text-xs font-semibold text-foreground">
                      KEY {track.key || "—"}
                    </span>
                    <span title="Duración total" className="rounded border border-border bg-background/80 px-2 py-0.5 text-xs font-semibold text-foreground">
                      {formatDuration(track.durationSec)}
                    </span>
                    <span title="Género dominante" className="rounded border border-border bg-background/80 px-2 py-0.5 text-xs font-semibold text-foreground">
                      {primaryGenre}
                    </span>
                  </div>
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <LicensingDialog
                      className="h-9 justify-center border border-foreground bg-foreground px-4 text-sm font-semibold text-background hover:opacity-90"
                      track={{
                        id: track.id,
                        title: track.title,
                        artist: track.artist,
                    ...
                    <Link
                      ...
                      className="inline-flex h-9 items-center justify-center gap-2 rounded border border-border bg-background px-3 text-sm font-semibold text-foreground transition hover:border-foreground/70"
                    >
            ...
            <article className="rounded-md border border-border bg-card/30 p-2.5">
              <PublicAudioBar
                src={audioSrc}
            ...
            <section className="rounded-md border border-border bg-card/30 p-3">
              ...
              <div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                {licenseOptions.map((option) => (
                  <article key={option.id} className="rounded border border-border bg-background/75 p-2.5">
                    <p className="truncate text-sm font-semibold text-foreground">{option.title}</p>
                    <p className="mt-1 text-base font-semibold text-foreground">{option.price}</p>
                    <p className="mt-0.5 truncate text-[10px] uppercase tracking-[0.12em] text-muted-foreground">{option.formats}</p>
                  </article>
                ))}
              </div>
            </section>

            <section id="entregables" className="rounded-md border border-border bg-card/30 p-3">
              <details open className="group">
                <summary className="cursor-pointer list-none text-[11px] font-semibold uppercase tracking-[0.14em] text-foreground/95">
                  Entregables del track
                </summary>
                <div className="mt-2 grid gap-2 md:grid-cols-2">
                  <article className="rounded border border-border bg-background/70 p-2.5">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Versiones</p>
                  ...
                  <article className="rounded border border-border bg-background/70 p-2.5">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Stems / Trackouts</p>
                    <ul className="mt-1.5 space-y-1 text-sm">
          ...
          <aside className="space-y-3 xl:sticky xl:top-[calc(var(--header-h)+0.75rem)]">
            <section className="rounded-md border border-border bg-card/35 p-3">
              <h3 className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-foreground/95">
                <SlidersHorizontal className="h-4 w-4" />
                Resumen comercial
              </h3>
              <div className="mt-2 rounded border border-border bg-background/70 px-2 py-1.5">
                <p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Precio guía</p>
                <p className="text-base font-semibold text-foreground">{guidePrice}</p>
              </div>
              <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 text-sm">
                <dt className="inline-flex items-center gap-1 text-muted-foreground">
                  Tipo licencia
                  <span title="Modelo de comercialización principal del beat">
                    <Info className="h-3 w-3" />
                  </span>
                </dt>
                <dd className="text-right font-semibold text-foreground">{formatLicenseType(track.licenseType)}</dd>
                <dt className="text-muted-foreground">Pricing tier</dt>
                <dd className="text-right font-semibold text-foreground">{formatPricingTier(track.pricingTier)}</dd>
                <dt className="inline-flex items-center gap-1 text-muted-foreground">
                  Presupuesto
                  <span title="Rango orientativo, el cierre puede variar por alcance real">
                    <Info className="h-3 w-3" />
                  </span>
                </dt>
